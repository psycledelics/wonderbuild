...
class ModTask(ProjectTask, _ModDepPhases):
	class Kinds(object):
		PROG = 0
	...
	def __call__(self, sched_ctx):
		self.private_deps = [] # of _ModDepPhases
		self.public_deps = [] # of _ModDepPhases
		self.cxx = None
	...
	result = property(_get_results, _set_results)
	...
	class _CallbackTask(Task):
		def __init__(self, mod_task): self.mod_task = mod_task
		__call__ = ModTask._callback
	
	def _callback(self, sched_context):
		...
		self.do_mod(sched_ctx)
		for dep in all_deps: dep.apply_cxx_to(self.cfg)
		if len(self.cfg.pkg_config) != 0:
			pkg_config_cxx_flags_task = _PkgConfigCxxFlagsTask.shared(self.project, self.cfg.pkg_config)
			sched_context.parallel_wait(pkg_config_cxx_flags_task)
			pkg_config_cxx_flags_task.apply_to(self.cfg)
