...
class ModTask(ModDepPhases, ProjectTask, Persistent):
	...
	def _mod_phase_callback(self, sched_ctx):
		...
		if False: # TODO generating pkg-config .pc file; work in progress
			...
			cxx_flags = self.cfg.impl.client_cfg_cxx_args(client_cfg)
			ld_flags = self.cfg.impl.client_cfg_ld_args(client_cfg)
			pkg_config_deps = self.cfg.pkg_config
			xxx_deps = self.topologically_sorted_unique_deep_deps(expose_private_deep_deps=None)
			print 'prefix=', fhs.dest.fs.root / fhs.prefix_path
			print 'exec_prefix=${prefix}/' + fhs.exec_prefix.rel_path(fhs.dest / fhs.prefix_path)
			print 'libdir=${exec_prefix}/' + fhs.lib.rel_path(fhs.exec_prefix)
			print 'includedir=${prefix}/' + fhs.include.rel_path(fhs.dest / fhs.prefix_path)
			print
			...
			print 'URL:', '...'
			print
			print 'Cflags:', '-I${includedir}', ' '.join(cxx_flags)
			print 'Libs:', '-L${libdir}', ' '.join(ld_flags)
			print 'Libs.private:', '...'
			print 'Requires:', ' '.join(pkg_config_deps)
			print 'Requires.private: # messy specification'
			print 'Conflicts:', '...'
