		...
		if need_process:
			...
			try: 
				...
				def generate(uninstalled, file):
					# Paths relative to a 'prefix' variable are *mandatory* for proper working on MS-Windows; see pkg-config(1) manpage.
					if False: # long comment about autotools ...
						# Traditionally, 3 more variables are defined, 'exec_prefix', 'libdir' and 'includedir',
						# whose values depend on that first 'prefix' variable.
						# For example:
						#
						# prefix=/usr/local
						# exec_prefix=${prefix}
						...
						# CFlags: -I${includedir}
						#
						# This is unnecessary with wonderbuild because take is cared to always write paths relative to the 'prefix' variable.
						# In wonderbuild this is simplified to:
						#
						# prefix=/usr/local
						#
						# Libs: -L${prefix}/lib
						...
						#
						# Following our example, with wonderbuild, if the user specified:
						# --install-dest-dir=/
						# --install-prefix-dir=/opt/tux
						# --install-lib-dir=/opt/tux/libraries
						#
						...
						#
						# By comparison, autoconf, in our example, is not generating a path relative to ${prefix}, but just '/opt/tux/libraries'.
						...
						#
						# ../configure --libdir=/opt/tux/libraries
						#
						# or invoke make with:
						#
						# make libdir=/opt/tux/libraries
						#
						# the content of foobar.pc will become:
						#
						# libdir=/opt/tux/libraries
						#
					...
					if self.url: s += '\nURL: ' + self.url
					s += \
						'\nCFlags: ' + ' '.join(self.cfg.impl.client_cfg_cxx_args(public_cfg, '${prefix}')) + \
						'\nLibs: ' + ' '.join(self.cfg.impl.client_cfg_ld_args(public_cfg, '${prefix}')) + \
						'\nLibs.private: ' + ' '.join(self.cfg.impl.client_cfg_ld_args(private_cfg, '${prefix}')) + \
