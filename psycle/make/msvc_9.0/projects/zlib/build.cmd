...
if not exist %output%\zlib-stamp (
	...
	xcopy /f %pkgdir%\%libdir%\zlibwapi.dll %output%\debug\bin\ || exit /b 1

	if not exist %output%\debug\lib (
		mkdir %output%\debug\lib || exit /b 1
	)
	xcopy /f %pkgdir%\%libdir%\zlibwapi.lib %output%\debug\lib\ || exit /b 1

	if not exist %output%\release\bin (
		mkdir %output%\release\bin || exit /b 1
	)
	xcopy /f %pkgdir%\%libdir%\zlibwapi.dll %output%\release\bin\ || exit /b 1

	if not exist %output%\release\lib (
		mkdir %output%\release\lib || exit /b 1
	)
	xcopy /f %pkgdir%\%libdir%\zlibwapi.lib %output%\release\lib\ || exit /b 1
