...
#ifndef PACKAGENERIC__PRE_COMPILED__INCLUDED
...
// os-specific
	...
	///\todo is gdi+ available with other compilers than microsoft's? by default, it's safer to assume it's not, as usual.
			...
			// the standard c++ header <algorithm> defines std::min and std::max,
			// but microsoft named them __cpp_min and __cpp_max, prefering their own macro stuff rather than the standard
